...
use quote::ToTokens;
use syn::{parse, parse2, parse_macro_input, spanned::Spanned, ItemEnum, ItemStruct, Lifetime, Type, Visibility};

use crate::{
	lifetimes::Lifetimes,
	prelude::{
		no_types::{MacroInnerAttr, MacroInnerAttrMeta, MacroInnerNoGen},
		MacroInner, ReturnType, Rule, RuleInner, RuleInnerEnum,
	},
};
...
pub fn rule(t: TokenStream) -> TokenStream {
	...
	for r in data.rules.0.iter() {
		out.extend(build_type(
			r,
			&data.meta.struct_vis,
			&data.meta.comp_type,
			data.meta.derived_traits.clone(),
		));
	...
	let (meta, rule, mut item) = if let Ok(i) = parse::<ItemStruct>(item.clone()) {
		...
		if let RuleInnerEnum::Multiple(_) = macro_inner.rule {
			return TokenStream::from(
				syn::Error::new(i.span(), "Cannot apply a rule with multiple variants to a struct").to_compile_error(),
			)
		}
		(
			macro_inner.meta,
		...
		for mut variant in i.variants {
			if let Some(p) = variant
				.attrs
				.iter()
				.position(|t| t.path.get_ident() == Some(&syn::Ident::new("parser", Span::mixed_site().into())))
			{
				...
				return TokenStream::from(
					syn::Error::new(variant.span(), "Enum variant must have a parser attribute macro")
						.to_compile_error(),
				)
			}
		}
		(
			...
			Rule {
				name: i.ident,
				inner: RuleInnerEnum::Multiple(rules),
			},
			TokenStream::from(new_item.to_token_stream()),
		)
	};
...
fn build_type(r: &Rule, vis: &Visibility, comp: &Type, traits: TokenStream) -> TokenStream {
	let mut out = if traits.is_empty() {
		...
		TokenStream::from_iter(vec![
			...
			group!(
				Delimiter::Bracket,
				vec![ident!("derive"), group!(Delimiter::Parenthesis; traits.clone())]
			),
		])
	};
...
fn build_impl(r: &Rule, tok: &Type, comp_type: &Type, map_fn: &TokenStream) -> TokenStream {
	...
	let mut out = TokenStream::from_iter(vec![ident!("impl"), punc!('<'), puncj!('\''), ident!("a")]);
	...
	let inner_match_stream = match &r.inner {
		RuleInnerEnum::Single(inner) => {
			...
			rule_inner.extend(vec![
				ident!("return"),
				ident!("Ok"),
				group!(Delimiter::Parenthesis, vec![ident!("Self"), return_args]),
			]);
			rule_inner
		}
		RuleInnerEnum::Multiple(inners) => {
			...
			rule_inner.extend(vec![punc!(';'), ident!("break")]);
			...
			rule_inner.extend(vec![ident!("Ok"), group!(Delimiter::Parenthesis; ok_inner)]);
			...
			local_out.extend(TokenStream::from_iter(vec![
				punc!(':'),
				group!(Delimiter::Brace; rule_inner),
				group!(
					Delimiter::Brace,
					vec![
						ident!("Ok"),
			...
			local_out.extend(vec![
				ident!("return"),
				ident!("Err"),
				group!(Delimiter::Parenthesis, Some(ident!("first_err"))),
			]);
			local_out
		}
	};
	...
	impl_fn.extend(vec![
		...
		group!(
			...
			vec![ident!("src"), punc!(':'), punc!('&'), ident!("mut"), ident!("T")]
		),
		puncj!('-'),
		punc!('>'),
		ident!("Result"),
	...
	impl_fn.extend(vec![
		punc!('>'),
		punc!('>'),
		group!(Delimiter::Brace; inner_match_stream),
	]);
